...
def model_risks(y, probabilities, classes):
    ...
    for i in range(len(y)):
        a = 1 if y[i] else 0
        model_risks.append(log_loss(y[i], probabilities[i][1]))
    return model_risks

# compute the true risk proxy using huge_dataset
true_risks = model_risks(huge_y, huge_probabilities, h.classes_)
...
# 5. Find results
differences = np.array(all_risks)[:,0] - np.array(len(all_risks) * [true_risk_proxy])
...
plt.ylabel('Density')
plt.savefig('homework-02/risk_differences.png', bbox_inches='tight')
